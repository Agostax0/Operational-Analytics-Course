...
class Attention(nn.Module):
    ...
    # initialize the weights (d=input dim, k=projection dim)
    def init_qkv(self, d, k):
        Wq, Wk, Wv = [nn.Parameter(torch.randn(d, k)) for _ in range(3)]
        return Wq, Wk, Wv

    # converts a torch 1-d array (X) into queries, keys, values
    def get_qkv(self, X):
...
class Encoder(nn.Module):
    def __init__(self, d, k, N):
        ...
        self.layers = nn.ModuleList(
            [EncoderBlock(d, k) for _ in range(N)]
        )
...
def plot_series(train_series, test_series=None, pred_series=None, title="", m="o"):
...
def train_loop(model, train_ds, optimizer, criterion):
    ...
    for X, Y in train_ds:
        ...
        output = model.generate(
            X.unsqueeze(-1), max_len=Y.size(1))
...
def train_model(model, train_ds, test_ds, epochs=400, lr=5e-4, batch_size=8):
    ...
    test_ds = torch.utils.data.DataLoader(test_ds, batch_size=batch_size)
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 100 == 0:
            forecast = model.generate(
                test_x[0].unsqueeze(
                    0).unsqueeze(-1), max_len=6).squeeze(
                0).detach().numpy()
...
def _init_panel(ax, split_name, x_data, in_len, out_len):
    ...
    panel = {
        "ctx": ax.plot(
            [], [], color="steelblue", marker=m, linewidth=2, label="Context")[0],
        "act": ax.plot(
            [], [], color="darkorange", marker=m, linewidth=2, label="Actual Future")[0],
        "pred": ax.plot(
            [], [], color="seagreen", marker=m, linewidth=2, label="Forecast")[0],
...
def _update_panel(model, ax, split_name, x_data, y_data, panel, frame_idx, in_len,
                  out_len):
    i = frame_idx % len(x_data)
    x_seq, y_true = x_data[i], y_data[i]
    with torch.no_grad():
        y_pred = model.generate(
            x_seq.unsqueeze(0).unsqueeze(-1), max_len=out_len).squeeze(0)
...
def plot_sliding_window_predictions(model, train_ds, test_ds):
    ...
    def update(frame_idx):
        ...
        for ax, (name, x_data, y_data) in zip(axes, splits):
            panel_vals, panel_artists = _update_panel(model, ax, name, x_data, y_data,
                                                      panels[name], frame_idx, in_len, out_len)
    ...
    with torch.no_grad():
        ...
        while remaining > 0:
            ...
            y_chunk = model.generate(context.unsqueeze(0).unsqueeze(-1),
                                     max_len=step).squeeze(
                0)
            preds.append(y_chunk)
            context = torch.cat([context, y_chunk], dim=
            0)[-x_seq.size(
                0):]
            remaining -= step
    return torch.cat(preds, dim=0).detach().cpu().numpy()

def forecast_all_inputs(model, input_len=12, out_len=6, train_size=0.8):
    series= get_airline_passenger_data()
    ...
    while remaining>0:
        ...
        context = torch.cat([context, torch.tensor(forecast_chunk,dtype=torch.float32)], dim=0)[-input_len:]
        remaining -= step
